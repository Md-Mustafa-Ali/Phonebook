                ...
                if(empty($name))
                 ...
                 { $conn=new mysqli("localhost","root","","phonebook");
                   if($conn->connect_error)
                    ...
                    { mysqli_query($conn,"DELETE FROM contact WHERE name='".$name."'");
                      echo '<script>alert("Contact deleted successfuly")</script>';
                    }
                 }
              }
             else
                ...
                elseif(val_e($email)==false)
                 ...
                 { $conn=new mysqli("localhost","root","","phonebook");
                   if($conn->connect_error)
                    ...
                    { mysqli_query($conn,"DELETE FROM contact WHERE name='".$email."'");
                      echo '<script>alert("Contact deleted successfuly")</script>';
                    }
                 }
              }
...
<div class="wrapper">
  <div>
    <div style="margin-left: 90px">
      <b><h2>DELETE CONTACT</h2></b>
    </div>
    <form action="" method="post">
      <label style="margin-left: 7px">Delete By :</label><br><br>
